...
export type TeleportOutsideData = {
    clickEntity?: Entity
    targetCoord: Vector2
    hoverText?: string
    trigger?: Trigger
...
export class Teleport {
    ...
    constructor(teleportData: TeleportData) {
        ...
        if (!teleportData.camaraTarget) {
            teleportData.camaraTarget = undefined
        }
...
@Component("TeleportOutside")
export class TeleportOutside {
    teleportData: TeleportOutsideData
    constructor(teleportData: TeleportOutsideData) {
        var self = this
        if (teleportData.hoverText === undefined || teleportData.hoverText === null) {
            teleportData.hoverText = "Teleport"
        }
    ...
    teleportPlayer(corrd: Vector2 = this.teleportData.targetCoord) {
        teleportTo(corrd.x + "," + corrd.y)
    }
}